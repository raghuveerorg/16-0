...
export default async function Profile() {
  ...
  const wrap = { maxWidth: 720, margin: "0 auto", padding: "24px 16px", color: "#eaf0ff", fontFamily: "system-ui, sans-serif" };
  if (!user) {
    return <main style={wrap}><p><Link href="/" style={{ color: "#ffb020" }}>← Sign in on the game page</Link> to see your history.</p></main>;
  }
  ...
  return (
    <main style={wrap}>
      ...
      {rows.length === 0 ? (
        <p style={{ color: "#9aa6cf" }}>No dailies yet — play today's challenge!</p>
      ) : (
        <table style={{ width: "100%", borderCollapse: "collapse", fontVariantNumeric: "tabular-nums" }}>
          <thead>
            <tr style={{ textAlign: "left", color: "#9aa6cf", fontSize: 13 }}>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.day} style={{ borderTop: "1px solid #2c3563" }}>
                <td style={{ padding: "8px 6px" }}>{r.day}</td>
                <td style={{ padding: "8px 6px", fontWeight: 700 }}>{r.wins}-{r.losses}</td>
                <td style={{ padding: "8px 6px" }}>#{r.rank} <span style={{ color: "#9aa6cf" }}>of {r.total}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
